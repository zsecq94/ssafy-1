...
- NOT NULL, UNIQUE, PRiMARY KEY(각 테이블에는 하나의기본 키만 있음), AUTOINCREMENT(사용되지 않은 값이나 이전에 삭제된 행의 값을 재사용하는 것을 방지)
...
- ALTER TABLE table_name RENAME TO new_table_name; : 테이블명 변경
- ALTER TABLE table_name RENAME COLUMN column_name TO new_column_name; : 컬럼명 변경
- ALTER TABLE table_name ADD COLUMN column_name; : 새 컬럼 추가
- ALTER TABLE table_name DROP COLUMN column_name; : 컬럼 삭제(컬럼이 다른 부분에서 참조되는 경우 삭제 불가)
